...
app.get("/usuarios", (req, res) => {
    try {
        client.query("SELECT * FROM Usuarios",
            (err, result) => {
                if (err) {
                    return console.error("Erro ao executar a qry de SELECT", err), alert("erro ao");
                }
...
app.get("/usuarios/:id", (req, res) => {
    try {
        ...
        client.query(
            ...
            (err, result) => {
                if (err) {
                    res.send("O dado: " + req.params.id + " é invalido");
                    return console.error("Erro ao executar a qry de SELECT id", err);
                }

                if (result.rowCount == 0)
                    res.send("Usuario " + req.params.id + " não existe");
